...
export default function ManageTripPage({ isGuest: propIsGuest }: ManageTripPageProps) {
  ...
  const handlePrint = async () => {
    ...
    setTimeout(async () => {
      ...
      const originalHidden = element.classList.contains("hidden");
      ...
      if (element.innerHTML.trim() === "" || !element.querySelector("h1, h2, table")) {
          ...
          if (originalHidden) element.classList.add("hidden");
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          toast({
            ...
            // Delay hiding to allow print dialog to process
            setTimeout(() => {
                if (originalHidden) element.classList.add("hidden"); else element.classList.remove("hidden"); // Restore original hidden state
                if (originalStyle) element.setAttribute("style", originalStyle); else element.removeAttribute("style");
                setReportDataForPrint(null);
            }, 1500); // Increased delay for hiding
        }
      } else {
        setReportDataForPrint(null);
      }
  ...
  const handleSaveFullTrip = async () => {
    ...
    const currentEditingTripId = searchParams.get('edit'); 
