...
export interface ManageTripPageProps {
  isGuest?: boolean;
}

export default function ManageTripPageClient({ isGuest: propIsGuest }: ManageTripPageProps) {
  ...
  React.useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      if (isMounted) {
        ...
        console.log("[ManageTripPage Debug] User set to:", activeUser ? activeUser.id : 'null', "Guest mode set to:", activeGuestMode);
      }
      ...
      if (tripIdToEditParam && !activeGuestMode && activeUser) {
        console.log(`[ManageTripPage Debug] Attempting to load trip ID: ${tripIdToEditParam} for editing for user: ${activeUser.id}`);
        try {
          ...
          if (fetchError && isMounted) {
            ...
            if (isMounted) {
                ...
                if (router && searchParams.get('edit')) router.replace('/manage-trip', undefined); 
            ...
            console.log("[ManageTripPage Debug] Raw transactions from DB for edit:", rawTransactions.length, JSON.stringify(rawTransactions).substring(0,100));
            ...
            console.log("[ManageTripPage Debug] Parsed and set transactions for edit:", parsedTransactions.length);
        ...
        if (isMounted) {
            console.log("[ManageTripPage Debug] Initializing for new trip or guest mode. Clearing previous active data.");
            setCurrentTripDetails(null);
            setTransactions([]);
            setExchangeRates(loadRatesFromLocal()); 
            if (tripDetailsFormRef.current) tripDetailsFormRef.current.resetForm(null);
            if (!activeGuestMode && activeUser && searchParams.get('edit')) { 
                 if (router) router.replace('/manage-trip', undefined); 
            }
  ...
  const handlePrint = async () => {
    ...
    setTimeout(async () => {
        ...
        element.setAttribute("style", "display: block !important; position: absolute !important; top: 0 !important; left: 0 !important; width: auto !important; height: auto !important; visibility: visible !important; z-index: 9999 !important; background: white !important;");
        ...
        if (element.innerHTML.trim() === "" || !element.querySelector("h1, h2, table")) {
            toast({
                variant: "destructive",
                title: "خطأ في محتوى التقرير",
                description: "محتوى التقرير فارغ أو غير جاهز للطباعة. يرجى المحاولة مرة أخرى.",
            });
            if (originalStyle) element.setAttribute("style", originalStyle); else element.removeAttribute("style");
            if (!reportDataForPrint) element.classList.add("hidden"); // Only add hidden if it was originally there
            setReportDataForPrint(null);
            return;
        }
        ...
        if (typeof window !== "undefined") {
            try {
                ...
                console.error("Error in handlePrint function (window.print):", error);
                toast({ variant: "destructive", title: "خطأ في الطباعة", description: error instanceof Error ? error.message : "حدث خطأ غير متوقع."});
            } finally {
                setTimeout(() => {
                    if (originalStyle) element.setAttribute("style", originalStyle); else element.removeAttribute("style");
                    if (!reportDataForPrint) element.classList.add("hidden");
                    setReportDataForPrint(null);
                }, 1500);
            }
        } else {
  ...
  const handleSaveFullTrip = async () => {
    ...
    const currentActiveEditingTripId = editingTripId; 

    if (!currentActiveEditingTripId && transactions.length === 0) {
    ...
    try {
      let error;
      if (currentActiveEditingTripId) {
        console.log(`[ManageTripPage Debug] Updating trip ID: ${currentActiveEditingTripId} on server for user ${user.id}.`);
        const { error: updateError } = await supabase.from('trips').update({ ...tripDataPayload, updated_at: new Date().toISOString() }).eq('id', currentActiveEditingTripId).eq('user_id', user.id);
        error = updateError;
      } else {
        console.log(`[ManageTripPage Debug] Inserting new trip to server for user ${user.id}.`);
      ...
      toast({ title: currentActiveEditingTripId ? "تم تحديث الرحلة بنجاح!" : "تم حفظ الرحلة بنجاح!", description: `تم ${currentActiveEditingTripId ? 'تحديث' : 'حفظ'} رحلة "${tripName}".` });
      ...
      console.error("[ManageTripPage Debug] Error saving/updating trip:", generalError);
    ...
    <div className="container mx-auto p-4 md:p-8 space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-6 no-print">
        ...
        <div className="flex flex-wrap gap-2 justify-center sm:justify-end">
          ...
          {!isGuestMode && (
            <Button onClick={() => router.push('/saved-trips')} variant="outline" asChild={false}>
